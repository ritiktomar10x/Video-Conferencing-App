  ...
  console.log(messages)
    ...
    let connectToSocketServer = () => {
        ...
        socketRef.current.on('connect', () => {
            // user login hai ya guest hai
            let token = localStorage.getItem("token");
            let isToken = false;
            if(token && token.length == 40){
                isToken = true;
            }
            socketRef.current.emit('join-call', window.location.href,isToken)
            socketIdRef.current = socketRef.current.id

            socketRef.current.on("nav-to-auth",(data)=>{
                navigate("/auth");
            })
                    ...
                    <div className="conferenceView">
                        ...
                        {videos.map((video) => (
                            <div key={video.socketId}>
                                <video className='meetUserVideo' style={{transform:"scaleX(-1)"}}
